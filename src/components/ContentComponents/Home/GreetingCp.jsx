...
import { greeting, socialNetwork } from "@/portfolioData/PortfolioData";

function GreetingCp({ textColor }) {
    const [svgFill, setvgFill] = useState(false);
    const idArray = [
        "path57",
        "path78",
        "path61",
    ...
    useEffect(() => {
        idArray.forEach((id) => {
            ...
            if (el) {
                el.setAttribute("fill", svgFill ? "" : "");
            }
        });
    }, [svgFill]);

    return (
        <div className={`flex justify-between ${textColor} transition-colors duration-300`}>
            <div className="pt-28">
                <h1 className="font-Noto-Sans-Display text-6xl font-bold ">{greeting.title}</h1>
                <div className="text-2xl pt-4 font-medium max-w-xl">
                    <p className="">
                        I'm <span className="text-text-red">{greeting.full_name}</span>
                        {greeting.subTitle}
                    </p>
                </div>
                <div className="socialNetwork flex justify-between max-w-50">
                    <div className="pt-4 text-4xl text-gray-500">
                        <Link to={socialNetwork.githupLink}>{socialNetwork.iconGithub}</Link>
                    </div>
                    <div className="pt-4 text-4xl text-cyanM">
                        <Link to={socialNetwork.linkEdinLink}>{socialNetwork.iconLinkedIn}</Link>
                    </div>
                    <div className="pt-4 text-4xl text-text-redGg">
                        <p>{socialNetwork.iconGooggle}</p>
                    </div>
                    <div className="pt-4 text-4xl text-blue-900">
                        <Link to={socialNetwork.facebookLink}>{socialNetwork.iconFacebook}</Link>
                    </div>
                </div>
                <div className="mt-8">
                    <Link
            ...
            <div className="greeting-image-div">
                ...
                >
                    <g style={{ filter: "drop-shadow(4px 6px 10px white" }}>
                        <path
